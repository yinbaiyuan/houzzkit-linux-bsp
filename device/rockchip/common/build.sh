...
echo "test"
               ...
               sed -i '/^#include "rp-lcd/ s/^/\/\//' $DTB_PATH                # commemt lcd dtsi that have included
...
function build_updateimg(){
	...
	if [ -f "$RK_PACKAGE_FILE_AB" ]; then
		...
		if [ "x$RK_TARGET_ROOTFS" = "xubuntu" ]; then
			RK_TARGET_ROOTFS=ubuntu-1804
		fi
		mv update.img $IMAGE_PATH/update-$RK_KERNEL_DTS-$MULTI_IMAGE$RK_TARGET_ROOTFS-$LCD_NAME-$DATE.img
	fi
