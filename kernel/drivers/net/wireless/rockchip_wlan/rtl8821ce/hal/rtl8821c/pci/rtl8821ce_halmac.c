...
static u8 pci_write_port_not_xmitframe(void *d,  u32 size, u8 *pBuf, u8 qsel)
{
	...
	if (qsel == HALMAC_TXDESC_QSEL_H2C_CMD) {
		...
		rtw_write32(padapter, REG_H2CQ_CSR_8821C,
			    rtw_read32(padapter, REG_H2CQ_CSR_8821C) | BIT8 | BIT16);
	} else {
		/* Set BCN BD Reg */
	...
	SET_TXBUFFER_DESC_ADD_LOW_WITH_OFFSET(txbd, 1,
				      (u32)(mapping + TX_WIFI_INFO_SIZE)); /* pkt */
...
#ifdef CONFIG_64BIT_DMA
	tmp = rtw_read16(padapter, REG_RX_RXBD_NUM_8821C);
	/* using 64bit */
