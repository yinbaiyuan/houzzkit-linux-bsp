...
static void update_status_success(){
    int fd, ret;
    
    const char open_flag [2]= "3";
    
    fd = open("/proc/rp_power/led", O_RDWR);
    ret = write(fd, open_flag, sizeof(open_flag));
    close(fd);

}


static void update_status_failed(){
    int fd, ret;
    
    const char open_flag [2]= "0";
...
main(int argc, char **argv) {
    ...
    ret = ui_init();
    printf("recovery ui_init return: %d\n", ret);
    if (ret >= 0) {
        ui_set_background(BACKGROUND_ICON_INSTALLING);
    } else {
        printf("recovery init ui fail, try to recovery process anyway.\n");
    }
    ...
    if(!bSDBoot) {
        ...
        if (is_sdcard_update()) {
            ...
            if (access(imageFile, F_OK) == 0) {
                ...
                read(fd, (char*)cmdline, 2048);
                close(fd);
                rp_sd_update_flag = strstr(cmdline, "sdfwupdate");
            }
        }
    }
    ...
    if (toggle_secure_fs) {
        ...
        #ifdef USE_UPDATEENGINE
        /**
         * use rkupdate to update image when use sd card to update
         */
        if (rp_sd_update_flag)
        {
            const char* binary = "/usr/bin/rkupdate";
            status = do_rk_update(binary, sdupdate_package);
        } else {
